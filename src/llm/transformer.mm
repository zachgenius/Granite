...
// NOTE: CoreML/ANE attention was tested and found to be 10-50x slower than Metal
// due to ~10ms fixed MPSGraph overhead per execution. CoreML is better suited
// for FFN offload (fixed shapes, larger batches) rather than attention.
// See roadmap.md Milestone 6.4 for investigation details.
...
Result<Tensor> TransformerModel::feed_forward_gpu(const Tensor& hidden, int layer) {
    ...
    // CoreML FFN for power-efficient inference
    // CoreML FFN for decode tokens (single token inference)
    // Note: Debug validation enabled to investigate Q4_K dequantization
    if (use_coreml_ffn_ && coreml_ffn_initialized_ && total_tokens == 1 && layer < (int)layer_weight_cache_.size()) {
        auto& cache = layer_weight_cache_[layer];
        if (cache.wgate_f16_buf && cache.wup_f16_buf && cache.wdown_f16_buf) {
            ...
            if (coreml && coreml->is_initialized()) {
                ...
                auto* wgate_f16 = static_cast<MTL::Buffer*>(cache.wgate_f16_buf);
                auto* wup_f16 = static_cast<MTL::Buffer*>(cache.wup_f16_buf);
                auto* wdown_f16 = static_cast<MTL::Buffer*>(cache.wdown_f16_buf);

                // CoreML FFN expects: input [batch, hidden], weights [hidden, inter] FP16
                auto result = coreml->forward(
                    h_buf, wgate_f16, wup_f16, wdown_f16, o_buf,
                    total_tokens, hidden_dim, intermediate_dim
                );
                ...
                if (result.ok()) {
                    ...
                    GRANITE_LOG_WARN("CoreML FFN failed for layer {}: {}, falling back to Metal",
                                    layer, result.error().message());
                    // Fall through to Metal path
                }
            }
        }
    }
#endif  // GRANITE_HAS_COREML
...
// =============================================================================
// CoreML FFN Integration
// =============================================================================

#ifdef GRANITE_HAS_COREML

// Q4_K block structure (same as in kernel_types.h but for CPU)
struct block_q4_K_cpu {
    uint16_t d;        // FP16 delta
    uint16_t dmin;     // FP16 min delta
    uint8_t scales[12];
    uint8_t qs[128];
};
...
struct block_q6_K_cpu {
    uint8_t ql[128];   // lower 4 bits of 6-bit quants (256 values, 2 per byte)
    uint8_t qh[64];    // upper 2 bits of 6-bit quants (256 values, 4 per byte)
    int8_t scales[16]; // 8-bit scales for each 16-element sub-block
...
// Extract scale and min for Q4_K dequantization
static inline void get_scale_min_k4_cpu(int j, const uint8_t* q, uint8_t& sc, uint8_t& m) {
    if (j < 4) {
        sc = q[j] & 63;
        m = q[j + 4] & 63;
    } else {
...
// Dequantize Q4_K weight matrix to FP16 with optional transpose
// Weight layout: [N, K] where K is input dim, N is output dim
// Q4_K: K must be multiple of 256 (super-block size)
// If transpose=true, output is [K, N] instead of [N, K]
static void dequantize_q4k_to_f16(
    const uint8_t* q4k_data,
    uint16_t* f16_data,
    int N,  // output dim (rows in source)
    int K,  // input dim (cols in source, must be multiple of 256)
    ...
    for (int row = 0; row < N; row++) {
        const block_q4_K_cpu* row_blocks = reinterpret_cast<const block_q4_K_cpu*>(q4k_data) + row * num_blocks_k;

        for (int kb = 0; kb < num_blocks_k; kb++) {
            const block_q4_K_cpu* block = &row_blocks[kb];
            float d = fp16_to_float(block->d);
            float dmin = fp16_to_float(block->dmin);

            // Process 256 elements per block
            int is = 0;
            for (int j = 0; j < 256; j += 64) {
                uint8_t sc1, m1, sc2, m2;
                get_scale_min_k4_cpu(is, block->scales, sc1, m1);
                get_scale_min_k4_cpu(is + 1, block->scales, sc2, m2);
                ...
                // Low nibble (elements j..j+31)
                for (int l = 0; l < 32; l++) {
                    float w = d1 * float(block->qs[j/2 + l] & 0xF) - dm1;
                    int col = kb * QK_K + j + l;
                    if (transpose) {
                        f16_data[col * N + row] = float_to_fp16(w);
                    } else {
                        f16_data[row * K + col] = float_to_fp16(w);
                    }
                }

                // High nibble (elements j+32..j+63)
                for (int l = 0; l < 32; l++) {
                    float w = d2 * float(block->qs[j/2 + l] >> 4) - dm2;
                    int col = kb * QK_K + j + 32 + l;
                    if (transpose) {
                        f16_data[col * N + row] = float_to_fp16(w);
                    } else {
                        f16_data[row * K + col] = float_to_fp16(w);
...
// Dequantize Q6_K weight matrix to FP16 with optional transpose
// Q6_K: 256 elements per block, 210 bytes per block
// Formula: w = d * scales[j/16] * (q6 - 32) where q6 is 6-bit value (0-63)
static void dequantize_q6k_to_f16(
    const uint8_t* q6k_data,
    uint16_t* f16_data,
    int N,  // output dim (rows in source)
    int K,  // input dim (cols in source, must be multiple of 256)
    bool transpose = false
    ...
    for (int row = 0; row < N; row++) {
        const block_q6_K_cpu* row_blocks = reinterpret_cast<const block_q6_K_cpu*>(q6k_data) + row * num_blocks_k;

        for (int kb = 0; kb < num_blocks_k; kb++) {
            const block_q6_K_cpu* block = &row_blocks[kb];
            float d = fp16_to_float(block->d);

            // Process 256 elements per block
            for (int j = 0; j < 256; j++) {
                // Get lower 4 bits from ql
                int ql_idx = j / 2;
                int ql_shift = (j % 2) * 4;
                uint8_t ql_val = (block->ql[ql_idx] >> ql_shift) & 0xF;

                // Get upper 2 bits from qh
                int qh_idx = j / 4;
                int qh_shift = (j % 4) * 2;
                uint8_t qh_val = (block->qh[qh_idx] >> qh_shift) & 0x3;

                // Combine to get 6-bit value
                int q6 = ql_val | (qh_val << 4);

                // Get scale for this 16-element sub-block
                int8_t scale = block->scales[j / 16];

                // Dequantize: w = d * scale * (q6 - 32)
                float w = d * float(scale) * float(q6 - 32);
...
Result<void> TransformerModel::init_coreml_ffn() {
    if (coreml_ffn_initialized_) return {};

    auto* gpu = get_metal_compute();
    if (!gpu || !gpu->is_initialized()) {
        return Error(ErrorCode::BackendNotInitialized, "Metal not initialized");
    }

    auto* coreml = get_coreml_ffn();
    if (!coreml) {
        return Error(ErrorCode::BackendNotInitialized, "CoreML FFN not available");
    }
    ...
    // Pre-compile FFN graph for this model's dimensions
    auto compile_result = coreml->compile_ffn(
        model_config_.hidden_dim,
        model_config_.intermediate_dim
    );
    if (!compile_result.ok()) {
        return compile_result;
    }

    GRANITE_LOG_INFO("Dequantizing FFN weights to FP16 for CoreML...");

    // Ensure layer weight cache is initialized
    if (!layer_cache_initialized_) {
        init_layer_weight_cache();
    }
    ...
    int num_layers = model_config_.num_layers;
    int hidden_dim = model_config_.hidden_dim;
    int inter_dim = model_config_.intermediate_dim;

    // Allocate FP16 buffers for each layer
    size_t gate_size = hidden_dim * inter_dim * sizeof(uint16_t);
    size_t up_size = hidden_dim * inter_dim * sizeof(uint16_t);
    size_t down_size = inter_dim * hidden_dim * sizeof(uint16_t);

    for (int layer = 0; layer < num_layers; layer++) {
        std::string prefix = "blk." + std::to_string(layer) + ".";
        auto& cache = layer_weight_cache_[layer];

        const RawWeight* wgate = get_raw_weight(prefix + "ffn_gate.weight");
        const RawWeight* wup = get_raw_weight(prefix + "ffn_up.weight");
        const RawWeight* wdown = get_raw_weight(prefix + "ffn_down.weight");
        ...
        // Check for supported quant types (Q4_K or Q6_K)
        auto is_supported = [](GGMLType t) {
            return t == GGMLType::Q4_K || t == GGMLType::Q6_K;
        };
        if (!is_supported(wgate->quant_type) ||
            !is_supported(wup->quant_type) ||
            !is_supported(wdown->quant_type)) {
            GRANITE_LOG_WARN("CoreML FFN requires Q4_K/Q6_K weights, but got gate={}, up={}, down={}",
                            ggml_type_name(wgate->quant_type),
                            ggml_type_name(wup->quant_type),
                            ggml_type_name(wdown->quant_type));
            return Error(ErrorCode::InvalidArgument,
                "CoreML FFN only supports Q4_K/Q6_K weights for all FFN tensors");
        }

        // Allocate FP16 buffers
        MTL::Buffer* gate_f16 = gpu->create_buffer(gate_size);
        MTL::Buffer* up_f16 = gpu->create_buffer(up_size);
        MTL::Buffer* down_f16 = gpu->create_buffer(down_size);

        if (!gate_f16 || !up_f16 || !down_f16) {
            if (gate_f16) gate_f16->release();
            if (up_f16) up_f16->release();
            if (down_f16) down_f16->release();
            return Error(ErrorCode::OutOfMemory,
                "Failed to allocate FP16 FFN weight buffers for layer " + std::to_string(layer));
        }

        // Get raw quantized data
        auto* wgate_buf = static_cast<MTL::Buffer*>(backend_->get_native_buffer(wgate->buffer));
        auto* wup_buf = static_cast<MTL::Buffer*>(backend_->get_native_buffer(wup->buffer));
        auto* wdown_buf = static_cast<MTL::Buffer*>(backend_->get_native_buffer(wdown->buffer));

        // GGUF stores weights for Y = X @ W^T, but CoreML does Y = X @ W
        // So we need to transpose all weights during dequantization:
        // - GGUF gate/up: [inter_dim, hidden_dim] -> CoreML [hidden_dim, inter_dim]
        // - GGUF down: [hidden_dim, inter_dim] -> CoreML [inter_dim, hidden_dim]

        // Dequantize gate weights [inter_dim, hidden_dim] -> transpose to [hidden_dim, inter_dim]
        if (wgate->quant_type == GGMLType::Q4_K) {
            dequantize_q4k_to_f16(
                static_cast<const uint8_t*>(wgate_buf->contents()),
                static_cast<uint16_t*>(gate_f16->contents()),
                inter_dim, hidden_dim, true  // transpose
            );
        } else {
            dequantize_q6k_to_f16(
                static_cast<const uint8_t*>(wgate_buf->contents()),
                static_cast<uint16_t*>(gate_f16->contents()),
                inter_dim, hidden_dim, true  // transpose
            );
        ...
        // Dequantize up weights [inter_dim, hidden_dim] -> transpose to [hidden_dim, inter_dim]
        if (wup->quant_type == GGMLType::Q4_K) {
            dequantize_q4k_to_f16(
                static_cast<const uint8_t*>(wup_buf->contents()),
                static_cast<uint16_t*>(up_f16->contents()),
                inter_dim, hidden_dim, true  // transpose
            );
        } else {
            dequantize_q6k_to_f16(
        ...
        // Dequantize down weights [hidden_dim, inter_dim] -> transpose to [inter_dim, hidden_dim]
        if (wdown->quant_type == GGMLType::Q4_K) {
            dequantize_q4k_to_f16(
                static_cast<const uint8_t*>(wdown_buf->contents()),
                static_cast<uint16_t*>(down_f16->contents()),
                hidden_dim, inter_dim, true  // transpose
            );
        } else {
            dequantize_q6k_to_f16(
                static_cast<const uint8_t*>(wdown_buf->contents()),
        ...
        cache.wgate_f16_buf = gate_f16;
        cache.wup_f16_buf = up_f16;
        cache.wdown_f16_buf = down_f16;

        GRANITE_LOG_DEBUG("Dequantized layer {} FFN weights to FP16", layer);
    }

    coreml_ffn_initialized_ = true;
    GRANITE_LOG_INFO("CoreML FFN initialized with {} layers", num_layers);
...
Result<void> TransformerModel::enable_coreml_ffn() {
#ifdef GRANITE_HAS_COREML
    if (!use_gpu_) {
        return Error(ErrorCode::InvalidState, "GPU must be enabled for CoreML FFN");
    }

    if (!CoreMLFFN::is_ane_available()) {
        return Error(ErrorCode::BackendNotInitialized, "ANE not available on this device");
    }

    auto result = init_coreml_ffn();
    if (!result.ok()) {
        return result;
    }

    use_coreml_ffn_ = true;
    GRANITE_LOG_INFO("CoreML FFN enabled (power-efficient mode)");
    return {};
#else
    return Error(ErrorCode::BackendNotInitialized, "CoreML not available (not built with GRANITE_HAS_COREML)");
#endif
}
