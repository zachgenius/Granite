...
// Use when activations are already in FP16 format (e.g., from fused RMSNorm).

kernel void matmul_q4k_simdgroup_f16(
    device const half* X           [[buffer(0)]],  // FP16 input activations
    device const void* W           [[buffer(1)]],
    device float* Y                [[buffer(2)]],
    constant uint& M               [[buffer(3)]],
    constant uint& K               [[buffer(4)]],
    ...
    for (uint loop_k = 0; loop_k < K; loop_k += SGMM_NK) {
        ...
        // FP16 input: read half2x4 directly (no float->half conversion)
        if (FC_mm_bc_inp) {
            if (y != nullptr) {
                const short sx = tiitg % NL1;
                const short sy = lr1 / 8;
...
// =============================================================================
// SIMDGROUP MATRIX FP16 Matmul (NK=64 Variant)
// =============================================================================
// Same as matmul_f16_simdgroup, but processes K in 64-element tiles.

kernel void matmul_f16_simdgroup_nk64(
    device const float* X          [[buffer(0)]],
    device const half* W           [[buffer(1)]],
    device float* Y                [[buffer(2)]],
    constant uint& M               [[buffer(3)]],
    ...
    constexpr short NL0 = SGMM_NK64 / 16;
    constexpr short NL1 = SGMM_NK64 / 8;
    ...
    for (uint loop_k = 0; loop_k < K; loop_k += SGMM_NK64) {
        half4x4 temp_a;
        if (FC_mm_bc_inp) {
            if (w_row != nullptr) {
                const uint k_base = loop_k + il0 * 16;
        ...
        #pragma clang loop unroll(full)
        for (short ik = 0; ik < SGMM_NK64 / 8; ik++) {
            simdgroup_barrier(mem_flags::mem_none);
            #pragma clang loop unroll(full)
            for (short i = 0; i < 4; i++) {
                simdgroup_load(ma[i], lsma + 64 * i, 8, 0, false);
...
// Both gate and up projections share the same input X, so we load X once
// and compute both outputs.
//
// Approach: Use tgpig.z to determine gate (z%2==0) or up (z%2==1) tile.
// For gate tiles: compute with W_gate, write to Y_gate
// For up tiles: compute with W_up, write to Y_up
// The input X tile loading is identical for both.

kernel void fused_gate_up_q4k_simdgroup(
    device const half* X           [[buffer(0)]],   // FP16 input [M, K]
    device const void* W_gate      [[buffer(1)]],   // Gate weights Q4_K [K, N]
    device const void* W_up        [[buffer(2)]],   // Up weights Q4_K [K, N]
    device float* Y_gate           [[buffer(3)]],   // Gate output [M, N]
    device float* Y_up             [[buffer(4)]],   // Up output [M, N]
    ...
    // Determine if this is a gate (z=0) or up (z=1) tile
    const bool is_up = (tgpig.z == 1);
    device const void* W = is_up ? W_up : W_gate;
    device float* Y = is_up ? Y_up : Y_gate;
...
// =============================================================================
// Fused Gate+Up Simdgroup Matmul for FFN (FP32 input)
// =============================================================================
// Same as fused_gate_up_q4k_simdgroup, but reads FP32 activations and converts
// to half in shared memory. Used when the FP16 RMSNorm path is unavailable.

kernel void fused_gate_up_q4k_simdgroup_f32(
    device const float* X          [[buffer(0)]],   // FP32 input [M, K]
    device const void* W_gate      [[buffer(1)]],   // Gate weights Q4_K [K, N]
    device const void* W_up        [[buffer(2)]],   // Up weights Q4_K [K, N]
    device float* Y_gate           [[buffer(3)]],   // Gate output [M, N]
    device float* Y_up             [[buffer(4)]],   // Up output [M, N]
